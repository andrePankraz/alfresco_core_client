# coding: utf-8

"""
    Alfresco Content Services REST API

    **Core API**  Provides access to the core features of Alfresco Content Services. 

    The version of the OpenAPI document: 1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from alfresco_core_api_client.models.rating_paging import RatingPaging

class TestRatingPaging(unittest.TestCase):
    """RatingPaging unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RatingPaging:
        """Test RatingPaging
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RatingPaging`
        """
        model = RatingPaging()
        if include_optional:
            return RatingPaging(
                list = alfresco_core_api_client.models.rating_paging_list.RatingPaging_list(
                    pagination = alfresco_core_api_client.models.pagination.Pagination(
                        count = 56, 
                        has_more_items = True, 
                        total_items = 56, 
                        skip_count = 56, 
                        max_items = 56, ), 
                    entries = [
                        alfresco_core_api_client.models.rating_entry.RatingEntry(
                            entry = alfresco_core_api_client.models.rating.Rating(
                                id = '', 
                                aggregate = alfresco_core_api_client.models.rating_aggregate.Rating_aggregate(
                                    number_of_ratings = 56, 
                                    average = 56, ), 
                                rated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                my_rating = '', ), )
                        ], )
            )
        else:
            return RatingPaging(
                list = alfresco_core_api_client.models.rating_paging_list.RatingPaging_list(
                    pagination = alfresco_core_api_client.models.pagination.Pagination(
                        count = 56, 
                        has_more_items = True, 
                        total_items = 56, 
                        skip_count = 56, 
                        max_items = 56, ), 
                    entries = [
                        alfresco_core_api_client.models.rating_entry.RatingEntry(
                            entry = alfresco_core_api_client.models.rating.Rating(
                                id = '', 
                                aggregate = alfresco_core_api_client.models.rating_aggregate.Rating_aggregate(
                                    number_of_ratings = 56, 
                                    average = 56, ), 
                                rated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                my_rating = '', ), )
                        ], ),
        )
        """

    def testRatingPaging(self):
        """Test RatingPaging"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
